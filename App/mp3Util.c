...
2017/3 Elizabeth Purswell changed it
*/
...
extern OS_EVENT *mboxB; // mailbox that sends from Mp3 task to LCD
extern OS_FLAG_GRP *rxFlags; // flags to synchronize mailboxes
...
void Mp3StreamSDFile(HANDLE hMp3, char *pFilename)
{
  ...
  // note: I just did this to see how Mp3GetRegister function works
  INT8U volbuf[10];
  memcpy(volbuf, BspMp3ReadVol, BspMp3ReadVolLen); // copy command from flash to a ram buffer
  Mp3GetRegister(hMp3, volbuf, BspMp3ReadVolLen);
  ...
  nextSong = OS_FALSE;
  INT8U buf[4]= {0,0,0,0};
  INT8U decodeTimeOld = 0;
  INT8U decodeTimeNew = 0;
  ...
  while (dataFile.available())
  {
    ...
    // now get the Decode Time
    memcpy(buf, BspMp3ReadDecodeTime, BspMp3ReadDecodeTimeLen); // copy command from flash to a ram buffer
    Mp3GetRegister(hMp3, buf, BspMp3ReadDecodeTimeLen);
    decodeTimeNew = buf[3];  // now current decode time is in buf[3]
    
    if(decodeTimeNew != decodeTimeOld)
    {
      PrintWithBuf(printBuf, PRINTBUFMAX, "Mp3StreamSDFile: File decode time is %d\n", decodeTimeNew);
      // send decode time back to LCD task in mboxB
      // LCD task is pending on flag bit 1 to be set
      // pend on flag bit 1 to be cleared (the decode time has been written to the LCD)
      OSMboxPost(mboxB,(void *)&decodeTimeNew);
      
      if(decodeTimeNew==1)
        OSTimeDly(10); // I had to add this little delay to get decode time = 1 to show on screen
      
      OSFlagPost(rxFlags,0x2,OS_FLAG_SET,&err); // set flag bit 1 to show that time is in mailbox
      OSFlagPend(rxFlags,0x2,OS_FLAG_WAIT_CLR_ALL,0,&err); // wait for flag to be cleared by LCD task
      
      // update decode time -- this is updated every second
      decodeTimeOld = decodeTimeNew;
    }
    ...
    if (OSFlagAccept(rxFlags,0x1,OS_FLAG_WAIT_CLR_ALL,&err))  // if STOP button was pressed, break out
    {
      break;
    }
  }
  
  //all done playing file (or STOP button was pressed)
  dataFile.close();
